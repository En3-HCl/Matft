...
public class MfArray<T: MfTypable>{
    public typealias MfType = T
    public internal(set) var mfdata: MfData<MfType> // Only setter is private
    ...
    public init (_ array: [Any], shape: [Int]? = nil, mforder: MfOrder = .Row) {
        ...
        var (flatten, _shape): ([MfType], [Int]) = array.withUnsafeBufferPointer{
        ...
        self.mfdata = MfData(flatten: &flatten)
        self.mfstructure = MfStructure(shapeptr: shapeptr, mforder: _mforder, ndim: shape.count)
    }
    public init (mfdata: MfData<MfType>, mfstructure: MfStructure){
        self.mfdata = mfdata
